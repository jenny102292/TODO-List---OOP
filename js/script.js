class TodoList {
	...
	addNewTask() {
		if (this.todoInput.value !== '' && (this.todoInput.value).trim()) {
			...
			this.alertInfo.innerText = 'Enter the content of the task!'
		} 
	}
	...
	changeTodo() {
		if (this.popupInput.value !== '') {
			...
			this.popupInfo.innerText = 'You must give some content!'
		}
	}
	...
	deleteTask(e) {
		...
		if(this.allTasks.length === 0) {
			this.alertInfo.textContent = 'No task on the list.'
		}
	}
